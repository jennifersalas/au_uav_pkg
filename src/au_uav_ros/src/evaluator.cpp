...
void Evaluator::setup(void) {
	//NOTE:With this change of adding parameters, evaluation must ALWAYS be ran with cmd line arguments.
	if (n.getParam("courseFile", filename)) {
        	ROS_ERROR("course filename is %s", filename.c_str());
    	} else {
        	ROS_ERROR("Please provide a course filename in cmd line arguments");
    	}

	if (n.getParam("outputFile", scoresheetFilename)) {
        	ROS_ERROR("scoresheetfilename is %s", scoresheetFilename.c_str());
    	} else {
        	ROS_ERROR("Please provide a output filename in cmd line arguments");
    	}
	...
	printf("Course File: %s\n", filename.c_str());
	printf("Score File: %s.score\n", scoresheetFilename.c_str());
...
void Evaluator::exitCallback(int sig) {
...
void Evaluator::endEvaluation(void)
{
	...
	else
	{
		...
		// print the course it was ran on
		fprintf(fp,"Course File: %s", filename.c_str());


		//dump our data into
		fprintf(fp, "\nPlane ID\tDistance Traveled(m)\tMinimum Travel(m)\t\tWaypoints Achieved\tTime of Death(s)\n");
		fprintf(fp, "--------\t--------------------\t-----------------\t\t------------------\t----------------\n");
		for(int id = 0; id <= lastPlaneID; id++)
		{
		...
		fprintf(allFP,"%s%s\n",scoresheetFilename.c_str(),".score");
...
void Evaluator::telemetry(const Telemetry &msg)
{
	...
	if(msg.planeID == maxAlivePlane)
	{
		...
		for(int id = 0; id <= lastPlaneID; id++)
		{
			...
			if (id == 24) {
			ROS_ERROR("wpQueue size: %d, distanceBtwn: %f", waypointQueues[id].size(), distanceBetween(waypointQueues[id].front(), current)); 
			}
