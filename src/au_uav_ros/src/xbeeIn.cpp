...
void XbeeIn::setup() {
	...
	sendAckClient = n.serviceClient<au_uav_ros::Ack>("ack_received");
	receiveAckService = n.advertiseService("add_ack", &XbeeIn::add_ack, this);
...
void* XbeeIn::serial_wait(void* serial_ptr)
{
	...
	while (ros::ok())
	{
		...
		{		
			...
			ROS_ERROR("MESSAGEID %d SEQ %d RECEIVED FROM PLANE %d", message.seq, message.msgid, message.sysid);
			...
			if(message.msgid == MAVLINK_MSG_ID_AU_UAV)
			{
				...
				if(tUpdate.currentLatitude == 0 && tUpdate.currentLongitude == 0)
				{
					tUpdate.currentLatitude += tUpdate.planeID;
					tUpdate.currentLongitude += tUpdate.planeID;
					tUpdate.distanceToDestination = 40;				
				}
			...
			if (message.msgid == MAVLINK_MSG_ID_MISSION_ACK) // message to acknowledge WP change
			{
				mavlink_mission_ack_t mission_ack;
				mavlink_msg_mission_ack_decode(&message, &mission_ack);
				/*Ack a;
				a.request.planeID = message.sysid;
				a.request.msgID = message.msgid;
				sendAckClient.call(a); //informs XbeeOut of ack received*/
			}
			if (message.msgid == MAVLINK_MSG_ID_PARAM_VALUE) // message to acknowledge param change
			{
				...
				a.request.planeID = message.sysid;
				a.request.msgID = message.msgid;
				a.request.param_value = (int)value_ack.param_value;
				sendAckClient.call(a); //informs XbeeOut of ack received
			}
