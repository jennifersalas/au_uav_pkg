...
void XbeeOut::setup() {
	...
	receiveAckService = n.advertiseService("ack_received", &XbeeOut::ack_received, this);

	//JJF
	resendCommandTimer = n.createTimer(ros::Duration(1), &XbeeOut::resendCommands, this);
...
// JJF 6/18/14: This method is called anytime a new command is posted to the commandTopic from Coordinator
// It puts the command onto the pendingAcks list and then sends a message that contains the command.
void XbeeOut::commands(const au_uav_ros::Command &cmd) {
	if (!cmd.sim) {
		sysid = cmd.planeID;
		if(cmd.commandID == COMMAND_SET_ID || cmd.commandID == COMMAND_SET_STREAMRATE)
		{
			//ROS_ERROR("Command %d pushed for plane %d", cmd.commandID, cmd.planeID);
		...
		XbeeOut::sendCommandMessage(cmd);
	}
	else {
...
// JJF 6/18/14: This method is used to pack a command into message form and send the message through MAVLINK
// It is used by XbeeOut::commands and XbeeOut::resendCommands
// It takes parameter cmd which is the command to be sent
void XbeeOut::sendCommandMessage(const au_uav_ros::Command &cmd)
{
	WPSendSeqNum++;
	sysid = cmd.planeID;
	...
	switch (cmd.commandID) {
		...
		case COMMAND_SET_ID: //change id commmand
			...
			break;
		case COMMAND_SET_STREAMRATE: //change stream rate command
			mavlink_msg_param_set_pack(serial_compid, compid, &mavlinkMsg, sysid, compid,
						"SR0_POSITION", (unsigned)cmd.param, MAVLINK_TYPE_UINT64_T);
			break;
		default:
			break;
	} 
...
// JJF 6/18/14: This method resends commands for which acknowledgements have not yet been received.
void XbeeOut::resendCommands(const ros::TimerEvent& event)
{
	if(!pendingAcks.empty()) //If there are pending acknowledgements
	{
		//ROS_ERROR("Scanning pending acknowledgements");
		std::list<Command>::iterator i;
		for (i = pendingAcks.begin(); i != pendingAcks.end(); i++) //iterates through pendingAcks
		{
			//checks if there has been enough delay since the message was last sent
			if((ros::Time::now().sec - i->commandHeader.stamp.sec) >= 2) 
			{
				// cmd gets the info from the old command
				au_uav_ros::Command cmd;
            			cmd.commandID = i->commandID;
	    			cmd.planeID = i->planeID;
	   			cmd.param = i->param;
	   			cmd.sim = false;
				// updates the listed timestamp on the command
				i->commandHeader.stamp = cmd.commandHeader.stamp = ros::Time::now();
				ROS_ERROR("RESENDING COMMAND %d to plane %d at time %d", cmd.commandID, cmd.planeID, cmd.commandHeader.stamp.sec);
				// resends the command				
				XbeeOut::sendCommandMessage(cmd);
			}		
		}
	}	
	else //If no pending acknowledgements do nothing	
	{	
		//ROS_ERROR("No pending commands");
	}
}
...
bool XbeeOut::ack_received(Ack::Request &req, Ack::Response &res) {
	//bool wpAckReceived;
	std::list<Command>::iterator i;
	for (i = pendingAcks.begin(); i != pendingAcks.end(); i++) {
		if (i->planeID == req.planeID) 
		{ //TODO use command seq to verify command
			if(req.msgID == 22 && (i->commandID == 3 || i->commandID == 4))
			{
				if(req.param_value == i->param)
				{
					ROS_ERROR("ACK RECEIVED, Command %d with param %d removed", i->commandID, i->param);					
					pendingAcks.erase(i);
					break;
				}
			}/*
			else if(req.msgID == 47 && (i->commandID == 1 || i->commandID == 2))			
			{								
				//wpAckReceived = true;
				pendingAcks.unique(same_plane_wp_command);
				pendingAcks.remove(pendingAcks[i]);
				break;
			}*/
		}
	}
	/*if (wpAckReceived) {
		ROS_ERROR("ACK RECEIVED, Command %d removed", i->commandID);
		pendingAcks.remove_if();
	}*/
	return true;
}
