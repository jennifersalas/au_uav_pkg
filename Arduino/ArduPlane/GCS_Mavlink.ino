...
// JJF 6/16/14: These macros are used for AU UAVs
#define CHECK_AU_UAV true
...
// JJF 6/16/14: this will return without doing anything if the usb is not connected or if the message is not a MSG_AU_UAV
//              the usb will only be used for the mission planner, so this allows us to not send out unneeded messages 
//              when the mission planner is not in use
#if CHECK_AU_UAV  
    if(!usb_connected && id != MSG_AU_UAV)
    {
      return;
    } 
#endif
...
{
  // JJF 6/16/14: if in use for AU UAVs only trigger for position and params
#if CHECK_AU_UAV  
    if (!usb_connected && stream_num != STREAM_PARAMS && stream_num != STREAM_POSITION) {
        return false; 
    }
#endif    
